...
The current implementation covers authenticated group messages and direct messages on handshake v3. Schema 10 adds policy configuration and metadata-only audit tables to the modern runtime. Historical policy schemas 2/3 are reconciled without discarding their configuration or audit.

1. The transport verifies the sender and recipient, decrypts the message, and checks group membership where applicable.
2. Inside one SQLite transaction, it stores the message, advances the ratchet/replay state, records reliable-delivery deduplication, and evaluates the policy gate. Audit persistence participates in that same transaction.
3. If any write fails, all writes roll back. No in-memory audit, policy dedup mark, decision, queue entry, or receipt is published. The sender can retry.
4. After commit, the runtime updates its audit ring and policy dedup set, emits the decision and metadata-only audit/activity events, then queues and emits allowed private inbound events. Existing `group:message` and `dm:message` consumers retain their payloads.
5. The delivery receipt acknowledges durable message storage, including when policy chooses `ignore`; it does not acknowledge execution or approval.

`PolicyGate.evaluate(event, afterCommit)` accepts an internal scheduling callback for transactional reception. Standalone calls without that callback apply their effects immediately. Do not schedule effects from a transaction that subsequently rolls back.

The inbound queue is bounded and process-local. Its contents are not restored after process restart. The durable audit preserves decisions, not an executable job queue. `act` and `ask` are decisions only: no tools execute and no approval UI is implied.

Raw `GroupManager` events and legacy message events are owner-private SDK surfaces, not public API endpoints. Only Agent-level `inbound:message` and `inboundQueue` are policy-filtered. The runtime isolates failures in its policy event observers so a faulty subscriber cannot suppress later subscribers or a delivery receipt.
...
`addressedToMe` is always true for an authenticated DM to this agent. For group messages it requires a token-bounded `@<fingerprint prefix>` or `@<displayName>` mention, unless `requireMention: false` is configured. Unknown DM senders produce `ask`, not automatic execution.
...
Schema 10 adds a `policy_audit` table with exactly these columns:
...
## Runtime configuration and owner access

The local CLI reads/writes SQLite without starting an Agent or opening network listeners. `L2S_DATA_DIR` expands `~` consistently with the SDK. CLI changes are read at the next Agent startup; use the owner's connected MCP controls to change the running Agent immediately.

MCP adds `get_pending_inbound_events` (drain 1–1000 events, default 50), `get_policy_audit_recent`, `get_policy_config`, `set_policy_config`, and trust/interest add/remove controls. Pending inbound events contain private plaintext encoded as base64 and optional UTF-8. Never publish them through the dashboard feed. Audit/config responses project explicit fields and omit injected properties.
