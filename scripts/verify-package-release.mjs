...
try {
  ...
  // git archive omits submodule contents. Export the exact pinned docs commit
  // so the frozen workspace install verifies the same graph as CI and Docker.
  const gitlinks = run('git', ['ls-tree', '-r', treeish], { encoding: 'utf8' })
    .split('\n').filter((line) => line.startsWith('160000 '));
  for (const gitlink of gitlinks) {
    const [, revision, relativePath] = gitlink.match(/^160000 commit ([0-9a-f]+)\t(.+)$/);
    const destination = join(source, relativePath);
    mkdirSync(destination, { recursive: true });
    const submoduleArchive = join(temporaryRoot, `submodule-${revision}.tar`);
    run('git', ['archive', '--format=tar', `--output=${submoduleArchive}`, revision], {
      cwd: join(repositoryRoot, relativePath),
    });
    run('tar', ['-xf', submoduleArchive, '-C', destination]);
  }
