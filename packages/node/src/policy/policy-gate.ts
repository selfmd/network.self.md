...
export class PolicyGate extends EventEmitter {
  ...
  evaluate(raw: unknown, afterCommit: (effect: () => void) => void = effect => effect()): GateOutcome {
    const receivedAt = this.now();

    // --- Step 1: structural validation ---
    const validation = validateInboundEvent(raw);
    if (!validation.ok) {
      const entry = this.recordReject(raw, receivedAt, validation.reason, afterCommit);
    ...
    if (this.dedup.has(ev.messageId)) {
      const entry = this.recordRejectFromEvent(ev, receivedAt, 'duplicate-event', afterCommit);
    ...
    if (ev.kind === 'group') {
      if (!ev.groupId) {
        ...
        const entry = this.recordRejectFromEvent(ev, receivedAt, 'malformed-event', afterCommit);
      ...
      try {
        ...
        const entry = this.recordRejectFromEvent(ev, receivedAt, 'not-a-member', afterCommit);
    ...
    this.audit.record(entry, afterCommit);

    // --- Step 6: mark dedup ONLY now (post-audit) ---
    afterCommit(() => this.markDedup(ev.messageId));
