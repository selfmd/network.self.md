...
describe('group epoch authorization before replay reservation', () => {
  ...
  function encryptedMessage(
    ...
    const generationId = new Uint8Array(16).fill(5);
    ...
    const aadParts = [
      new TextEncoder().encode('network.self.md/GroupMessage/v2\0'),
      groupId,
      sender.edPublicKey,
      generationId,
      epochHash,
    ...
    const aad = new Uint8Array(
      aadParts.reduce((length, part) => length + part.length, 0),
    );
    let offset = 0;
    for (const part of aadParts) {
      aad.set(part, offset);
      offset += part.length;
    }
