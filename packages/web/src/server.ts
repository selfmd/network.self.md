...
export class TTYAServer {
  ...
  constructor(
    config: Partial<TTYAServerConfig> &
      Pick<
        TTYAServerConfig,
        'agentFingerprint' | 'agentEdPublicKey' | 'ttyaAuthSecret'
      >,
  ) {
    if (
      !(config.agentEdPublicKey instanceof Uint8Array) ||
      config.agentEdPublicKey.length !== 32
    ) {
    ...
    this.config = {
      ...DEFAULT_CONFIG,
      ...config,
      agentEdPublicKey: new Uint8Array(config.agentEdPublicKey),
      ttyaAuthSecret: copyAndValidateTTYAAuthSecret(config.ttyaAuthSecret),
    };

    this.bridge = new TTYABridge(
      this.config.agentEdPublicKey,
      this.config.ttyaAuthSecret,
    );
    ...
    app.get<{ Params: { fingerprint: string } }>(
      ...
      (socket, req) => {
        ...
        socket.on('message', (raw: Buffer | string) => {
          ...
          const minInterval =
            (this.config.rateLimit.perSeconds * 1000) /
            this.config.rateLimit.messages;
          ...
          if (!firstMessageHandled) {
            ...
            const session = this.queue.addVisitor(
              visitorId,
              clientMsg.content,
              ipHash,
              socket as any,
