...
describe('protocol runtime validation', () => {
  ...
  it.each([
    ...
    ['bad envelope version', { ...goldenMessages[2], protocolVersion: 1 }],
...
describe('authenticated protocol payloads', () => {
  ...
  it('binds the sender-key generation to the group-message signature', () => {
    const message = signedMessages()[0] as GroupEncryptedMessage;
    expect(
      verifyAuthenticatedMessage(
        { ...message, generationId: bytes(16, 7) },
        sender.edPublicKey,
      ),
    ).toBe(false);
  });

  it('binds invite and epoch context to the management signature', () => {
    const message = signAuthenticatedMessage<GroupManagementMessage>(
      {
        type: MessageType.GroupManagement,
        groupId: bytes(32, 1),
        action: 'accept',
        targetFingerprint: sender.fingerprint,
        inviteId: 'invite-1',
        epochVersion: 3,
        epochHash: bytes(32, 8),
        senderFingerprint: sender.fingerprint,
    ...
    for (const swapped of [
      { ...message, inviteId: 'invite-2' },
      { ...message, epochVersion: 4 },
      { ...message, epochHash: bytes(32, 9) },
    ]) {
      expect(verifyAuthenticatedMessage(swapped, sender.edPublicKey)).toBe(false);
    }
  });
